          ...
          if (quoteItems.length > 0) {
            ...
            setItems(quoteItems.map((qi, idx) => {
              ...
              return {
                ...
                spec: qi.spec || qi.grade || matchedProd?.spec || '',
                qty,
                unit: qi.unit || 'kg',
                ...
                palletQty: qi.palletQty || 0,
                selectedPackingMethodId: qi.selectedPackingMethodId || '',
                packingSpecOverride: qi.packingSpecOverride || null,
                remarks: qi.remarks || '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const orderPayload: Order = {
        ...
        items: items.map(it => {
          const itemPurchaseCurr = it.purchaseUnitCurrency || it.purchasePriceCurrency || (it.currency === 'KRW' ? 'KRW' : 'USD');
          const pUnitPrice = parseFloat(it.purchaseUnitPrice as any) || 0;
          return {
            itemId: it.itemId || '',
            ...
            purchaseUnitPrice: pUnitPrice,
            purchaseUnitCurrency: itemPurchaseCurr as any,
            purchasePriceCurrency: itemPurchaseCurr,
            purchasePriceKrw: itemPurchaseCurr === 'KRW' ? pUnitPrice : (parseFloat(it.purchasePriceKrw as any) || 0),
            purchasePriceUsd: itemPurchaseCurr === 'USD' ? pUnitPrice : (parseFloat(it.purchasePriceUsd as any) || 0),
            exchangeRate: parseFloat(it.exchangeRate as any) || (formData.exchangeRate || 1400),
            marginRate: it.marginRate != null ? (parseFloat(it.marginRate as any) || 0) : 15,
            roundDigits: it.roundDigits,
            ...
            originalPurchasePrice: formData.quotationId ? (parseFloat(it.originalPurchasePrice as any) || 0) : 0,
            originalPurchaseCurrency: formData.quotationId ? (it.originalPurchaseCurrency || itemPurchaseCurr) : itemPurchaseCurr,
            amount: it.amount || 0,

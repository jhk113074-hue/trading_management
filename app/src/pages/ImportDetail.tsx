    ...
    <div style={{ padding: '24px', background: '#f8fafc', minHeight: 'calc(100vh - 64px)', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <h2 style={{ fontSize: '20px', fontWeight: 800, color: '#1e293b', margin: '0 0 6px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
          {viewMode === 'quote' ? '수입 견적관리' : '수입 주문관리'}
...
customsDuty,
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #cbd5e1', paddingBottom: '8px', marginBottom: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#eff6ff', padding: '3px 8px', borderRadius: '4px', border: '1px solid #bfdbfe' }}>
                    <span style={{ fontSize: '11px', fontWeight: 750, color: '#1e40af' }}>⚡ 전체 품목 일괄 올림/반올림:</span>
                    <select
                      onChange={(e) => {
                        const rule = e.target.value;
                        if (!rule) return;
                        const defaultFx = request.costBreakdown?.appliedExchangeRate || 1450;
                        const nextItems = (request.piItems || []).map(it => {
                          const bCur = it.buyingCurrency || it.currency || 'USD';
                          const qCur = it.quoteCurrency || it.currency || (bCur === 'KRW' ? 'KRW' : 'USD');
                          const fxRate = Number(it.exchangeRate !== undefined && it.exchangeRate !== '' ? it.exchangeRate : defaultFx);
                          ...
                          let baseInQCur = bPrice;
                          if (bCur !== qCur) {
                            if (bCur === 'USD' && qCur === 'KRW') baseInQCur = bPrice * fxRate;
                            else if (bCur === 'KRW' && qCur === 'USD') baseInQCur = fxRate > 0 ? bPrice / fxRate : bPrice;
                            else baseInQCur = bPrice * fxRate;
                          }

                          const rawQPrice = baseInQCur * (1 + mRate / 100);
                          const qPrice = roundByDigit(rawQPrice, rule, qCur === 'KRW');
                          const qAmt = ((Number(it.qty) || 0) * qPrice).toFixed(2);
                          const recalculatedMargin = baseInQCur > 0 ? ((qPrice - baseInQCur) / baseInQCur) * 100 : mRate;
                          return {
                            ...it,
                            roundingMode: rule,
                        ...
                        const nextCombinedKrw = Math.round(totalQUsd * defaultFx) + freightAmtKrw;
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                  <thead>
                    <tr style={{ background: '#f8fafc', borderBottom: '1px solid #cbd5e1', height: '34px' }}>
                      <th style={{ padding: '6px 3px', textAlign: 'center', width: '28px', fontSize: '11px', fontWeight: 750, color: '#475569' }}>No</th>
                      <th style={{ padding: '6px 4px', textAlign: 'left', fontSize: '11px', fontWeight: 750, color: '#475569' }}>품명 (COMMODITY)</th>
                      <th style={{ padding: '6px 3px', textAlign: 'left', width: '65px', fontSize: '11px', fontWeight: 750, color: '#475569' }}>HS CODE</th>
                      <th style={{ padding: '6px 3px', textAlign: 'right', width: '50px', fontSize: '11px', fontWeight: 750, color: '#475569' }}>수량</th>
                      <th style={{ padding: '6px 3px', textAlign: 'center', width: '45px', fontSize: '11px', fontWeight: 750, color: '#475569' }}>단위</th>
                      <th style={{ padding: '6px 3px', textAlign: 'center', width: '70px', fontSize: '11px', fontWeight: 750, color: '#475569', background: '#f1f5f9' }}>매입통화</th>
                      <th style={{ padding: '6px 3px', textAlign: 'right', width: '85px', fontSize: '11px', fontWeight: 750, color: '#475569', background: '#f1f5f9' }}>매입단가 (BUY)</th>
                      <th style={{ padding: '6px 3px', textAlign: 'right', width: '70px', fontSize: '11px', fontWeight: 750, color: '#b45309', background: '#fef3c7' }}>기준환율</th>
                      <th style={{ padding: '6px 3px', textAlign: 'right', width: '55px', fontSize: '11px', fontWeight: 750, color: '#1d4ed8', background: '#eff6ff' }}>마진율(%)</th>
                      <th style={{ padding: '6px 3px', textAlign: 'center', width: '110px', fontSize: '11px', fontWeight: 750, color: '#2563eb', background: '#eff6ff' }}>올림/반올림</th>
                      <th style={{ padding: '6px 3px', textAlign: 'center', width: '70px', fontSize: '11px', fontWeight: 750, color: '#1e3a8a', background: '#dbeafe' }}>견적통화</th>
                      <th style={{ padding: '6px 3px', textAlign: 'right', width: '95px', fontSize: '11px', fontWeight: 750, color: '#1e3a8a', background: '#dbeafe' }}>견적단가 (QUOTE)</th>
                      <th style={{ padding: '6px 4px', textAlign: 'right', width: '105px', fontSize: '11px', fontWeight: 750, color: '#1e3a8a' }}>견적총액 (AMOUNT)</th>
                      <th style={{ padding: '6px 2px', textAlign: 'center', width: '25px' }}></th>
                    </tr>
                  </thead>
                  <tbody>
                    {(request.piItems || []).map((item, idx) => {
                      const defaultFx = request.costBreakdown?.appliedExchangeRate || 1450;
                      const bCur = item.buyingCurrency || item.currency || 'USD';
                      const qCur = item.quoteCurrency || item.currency || (bCur === 'KRW' ? 'KRW' : 'USD');
                      const bSym = bCur === 'KRW' ? '₩' : bCur === 'EUR' ? '€' : bCur === 'RMB' ? '¥' : '$';
                      const qSym = qCur === 'KRW' ? '₩' : qCur === 'EUR' ? '€' : qCur === 'RMB' ? '¥' : '$';
                      const fxRate = Number(item.exchangeRate !== undefined && item.exchangeRate !== '' ? item.exchangeRate : defaultFx);
                      const buyingUPrice = Number(item.buyingUnitPrice !== undefined ? item.buyingUnitPrice : item.unitPrice) || 0;
                      const marginVal = Number(item.marginRate !== undefined ? item.marginRate : (request.quoteProductMarginRate ?? 10));
                      const itemRule = item.roundingMode || 'NONE';

                      let baseInQCur = buyingUPrice;
                      if (bCur !== qCur) {
                        if (bCur === 'USD' && qCur === 'KRW') baseInQCur = buyingUPrice * fxRate;
                        else if (bCur === 'KRW' && qCur === 'USD') baseInQCur = fxRate > 0 ? buyingUPrice / fxRate : buyingUPrice;
                        else if (bCur === 'EUR' && qCur === 'KRW') baseInQCur = buyingUPrice * fxRate;
                        else if (bCur === 'RMB' && qCur === 'KRW') baseInQCur = buyingUPrice * fxRate;
                        else baseInQCur = buyingUPrice * fxRate;
                      }

                      const rawQuote = baseInQCur * (1 + marginVal / 100);
                      const defaultQuoteUPrice = roundByDigit(rawQuote, itemRule, qCur === 'KRW');
                        ...
                        <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', height: '36px' }}>
                          ...
                          <td style={{ padding: '2px 3px' }}>
                            <input
                              ...
                              style={{ width: '100%', height: '26px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11.5px', padding: '0 4px', outline: 'none', boxSizing: 'border-box' }}
                            />
                          </td>
                          <td style={{ padding: '2px 3px' }}>
                            <input
                          ...
                          <td style={{ padding: '2px 3px' }}>
                            <select
                              ...
                              style={{ width: '100%', height: '26px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11px', padding: '0 2px', outline: 'none', boxSizing: 'border-box' }}
                            >
                              <option value="EA">EA</option>
                              <option value="KG">KG</option>
                              <option value="ROLL">ROLL</option>
                          ...
                          {/* 1. 매입통화 선택 */}
                          <td style={{ padding: '2px 3px', background: '#f8fafc' }}>
                            <select
                              value={bCur}
                              onChange={(e) => {
                                const nextBCur = e.target.value;
                                let newBaseInQ = buyingUPrice;
                                if (nextBCur !== qCur) {
                                  if (nextBCur === 'USD' && qCur === 'KRW') newBaseInQ = buyingUPrice * fxRate;
                                  else if (nextBCur === 'KRW' && qCur === 'USD') newBaseInQ = fxRate > 0 ? buyingUPrice / fxRate : buyingUPrice;
                                  else newBaseInQ = buyingUPrice * fxRate;
                                }
                                const nextRaw = newBaseInQ * (1 + marginVal / 100);
                                const nextQPrice = roundByDigit(nextRaw, itemRule, qCur === 'KRW');
                                const nextAmt = ((Number(item.qty) || 0) * nextQPrice).toFixed(2);

                                const nextItems = [...(request.piItems || [])];
                                nextItems[idx] = { 
                                  ...item, 
                                  buyingCurrency: nextBCur,
                                  currency: nextBCur,
                                  quoteUnitPrice: String(nextQPrice),
                                  amount: nextAmt
                                };
                                saveToStorage(importRequests.map(r => r.id === id ? { ...r, piItems: nextItems } : r));
                              }}
                              style={{ width: '100%', height: '26px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11px', fontWeight: 700, color: bCur === 'KRW' ? '#047857' : '#1d4ed8', background: '#fff', padding: '0 2px', outline: 'none', boxSizing: 'border-box' }}
                            >
                              <option value="USD">USD ($)</option>
                              <option value="KRW">KRW (₩)</option>
                              <option value="EUR">EUR (€)</option>
                          ...
                          <td style={{ padding: '2px 3px', background: '#f8fafc' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                              <span style={{ fontSize: '11px', fontWeight: 600, color: '#64748b' }}>{bSym}</span>
                              <input
                                type="number"
                                ...
                                onChange={(e) => {
                                  ...
                                  let newBaseInQ = bPrice;
                                  if (bCur !== qCur) {
                                    if (bCur === 'USD' && qCur === 'KRW') newBaseInQ = bPrice * fxRate;
                                    else if (bCur === 'KRW' && qCur === 'USD') newBaseInQ = fxRate > 0 ? bPrice / fxRate : bPrice;
                                    else newBaseInQ = bPrice * fxRate;
                                  }
                                  const rawQ = newBaseInQ * (1 + marginVal / 100);
                                  const qPrice = roundByDigit(rawQ, itemRule, qCur === 'KRW');
                                ...
                                style={{ width: '100%', height: '26px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11.5px', padding: '0 4px', outline: 'none', textAlign: 'right', boxSizing: 'border-box' }}
                              />
                            </div>
                          </td>

                          {/* 3. 기준환율 (매입통화와 견적통화가 다를 때 적용) */}
                          <td style={{ padding: '2px 3px', background: '#fef3c7' }}>
                            <input
                              type="number"
                              value={item.exchangeRate !== undefined ? item.exchangeRate : defaultFx}
                              onChange={(e) => {
                                const val = e.target.value;
                                const newFx = Number(val) || defaultFx;
                                let newBaseInQ = buyingUPrice;
                                if (bCur !== qCur) {
                                  if (bCur === 'USD' && qCur === 'KRW') newBaseInQ = buyingUPrice * newFx;
                                  else if (bCur === 'KRW' && qCur === 'USD') newBaseInQ = newFx > 0 ? buyingUPrice / newFx : buyingUPrice;
                                  else newBaseInQ = buyingUPrice * newFx;
                                }
                                const rawQ = newBaseInQ * (1 + marginVal / 100);
                                const qPrice = roundByDigit(rawQ, itemRule, qCur === 'KRW');
                                const qAmt = ((Number(item.qty) || 0) * qPrice).toFixed(2);
                              ...
                              style={{ width: '100%', height: '26px', border: bCur !== qCur ? '1.5px solid #d97706' : '1px solid #fde68a', borderRadius: '4px', fontSize: '11.5px', fontWeight: 700, color: '#92400e', background: '#fff', padding: '0 3px', outline: 'none', textAlign: 'right', boxSizing: 'border-box' }}
                              title={bCur !== qCur ? `환율 적용: ${bCur} ➔ ${qCur}` : '동일 통화'}
                            />
                          </td>

                          {/* 4. 마진율 (%) */}
                          <td style={{ padding: '2px 3px', background: '#eff6ff' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                              <input
                                type="number"
                                ...
                                onChange={(e) => {
                                  ...
                                  const rawQ = baseInQCur * (1 + mRate / 100);
                                  const qPrice = roundByDigit(rawQ, itemRule, qCur === 'KRW');
                                  const qAmt = ((Number(item.qty) || 0) * qPrice).toFixed(2);
                                ...
                                style={{ width: '100%', height: '26px', border: '1px solid #bfdbfe', borderRadius: '4px', fontSize: '11px', fontWeight: 700, color: '#1d4ed8', padding: '0 2px', outline: 'none', textAlign: 'right', boxSizing: 'border-box' }}
                              />
                              <span style={{ fontSize: '10px', color: '#1d4ed8', fontWeight: 700 }}>%</span>
                            </div>
                          </td>

                          {/* 5. 올림/반올림 자리수 선택 드롭다운 */}
                          <td style={{ padding: '2px 3px', background: '#eff6ff' }}>
                            <select
                              value={item.roundingMode || 'NONE'}
                              onChange={(e) => {
                                const rule = e.target.value;
                                const rawQ = baseInQCur * (1 + marginVal / 100);
                                const qPrice = roundByDigit(rawQ, rule, qCur === 'KRW');
                                const qAmt = ((Number(item.qty) || 0) * qPrice).toFixed(2);
                                const recalculatedMargin = baseInQCur > 0 ? ((qPrice - baseInQCur) / baseInQCur) * 100 : marginVal;
                              ...
                              style={{ width: '100%', height: '26px', border: '1px solid #bfdbfe', borderRadius: '4px', fontSize: '11px', fontWeight: 700, color: '#1e40af', background: '#fff', padding: '0 2px', outline: 'none', cursor: 'pointer', boxSizing: 'border-box' }}
                            >
                              <option value="NONE">소수점 (기본)</option>
                          ...
                          {/* 6. 견적(판매)통화 선택 */}
                          <td style={{ padding: '2px 3px', background: '#dbeafe' }}>
                            <select
                              value={qCur}
                              onChange={(e) => {
                                const nextQCur = e.target.value;
                                let newBaseInQ = buyingUPrice;
                                if (bCur !== nextQCur) {
                                  if (bCur === 'USD' && nextQCur === 'KRW') newBaseInQ = buyingUPrice * fxRate;
                                  else if (bCur === 'KRW' && nextQCur === 'USD') newBaseInQ = fxRate > 0 ? buyingUPrice / fxRate : buyingUPrice;
                                  else newBaseInQ = buyingUPrice * fxRate;
                                }
                                const nextRaw = newBaseInQ * (1 + marginVal / 100);
                                const nextQPrice = roundByDigit(nextRaw, itemRule, nextQCur === 'KRW');
                                const nextAmt = ((Number(item.qty) || 0) * nextQPrice).toFixed(2);

                                const nextItems = [...(request.piItems || [])];
                                nextItems[idx] = { 
                                  ...item, 
                                  quoteCurrency: nextQCur,
                                  quoteUnitPrice: String(nextQPrice),
                                  amount: nextAmt
                                };
                                saveToStorage(importRequests.map(r => r.id === id ? { ...r, piItems: nextItems } : r));
                              }}
                              style={{ width: '100%', height: '26px', border: '1.5px solid #3b82f6', borderRadius: '4px', fontSize: '11px', fontWeight: 800, color: qCur === 'KRW' ? '#047857' : '#1e3a8a', background: '#fff', padding: '0 2px', outline: 'none', boxSizing: 'border-box' }}
                            >
                              <option value="USD">USD ($)</option>
                              <option value="KRW">KRW (₩)</option>
                              <option value="EUR">EUR (€)</option>
                          ...
                          <td style={{ padding: '2px 3px', background: '#dbeafe' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                              <span style={{ fontSize: '11px', fontWeight: 750, color: '#1e3a8a' }}>{qSym}</span>
                              <input
                                type="number"
                                value={item.quoteUnitPrice !== undefined && item.quoteUnitPrice !== '' ? item.quoteUnitPrice : defaultQuoteUPrice}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  const qPrice = Number(val) || 0;
                                  const mRate = baseInQCur > 0 ? ((qPrice - baseInQCur) / baseInQCur) * 100 : 0;
                                  const qAmt = ((Number(item.qty) || 0) * qPrice).toFixed(2);
                                ...
                                style={{ width: '100%', height: '26px', border: '1.5px solid #2563eb', borderRadius: '4px', fontSize: '12px', fontWeight: 800, color: '#1e3a8a', padding: '0 4px', outline: 'none', textAlign: 'right', boxSizing: 'border-box', background: '#fff' }}
                              />
                            </div>
                          </td>

                          {/* 8. 견적총액 (TOTAL AMOUNT) */}
                          <td style={{ textAlign: 'right', fontWeight: 800, color: qCur === 'KRW' ? '#047857' : '#1e3a8a', paddingRight: '8px' }}>
                            {qSym} {Number(totalQuoteAmt || 0).toLocaleString(undefined, { minimumFractionDigits: qCur === 'KRW' ? 0 : 2, maximumFractionDigits: qCur === 'KRW' ? 0 : 2 })}
                          </td>
                          <td style={{ textAlign: 'center' }}>
                            <button
                              type="button"
                ...
                {(request.payments || []).length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '30px', color: '#64748b', fontSize: '13px', fontWeight: 600 }}>
                    ...
                    <div key={pay.id} style={{ background: '#fff', border: '1px solid #cbd5e1', borderRadius: '6px', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: '8px', boxShadow: '0 1px 3px rgba(0,0,0,0.03)' }}>
                      ...
                      <div style={{ background: '#f8fafc', border: '1px dashed #cbd5e1', borderRadius: '4px', padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #e2e8f0', paddingBottom: '4px' }}>
                          <span style={{ fontSize: '11px', fontWeight: 800, color: '#1e3a8a' }}>
                            🏦 외환거래계산서 수수료 및 환율 개별 관리
                          </span>
                          {totalFeeKrwThisRound > 0 && (
                            <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', padding: '2px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 750, color: '#1d4ed8' }}>
                              회차 수수료 합계: ₩{totalFeeKrwThisRound.toLocaleString()}원
                            </div>
                          )}
                        </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                            ...
                            <input 
                              ...
                              onChange={(e) => {
                                ...
                                const updatedPayments = (request.payments || []).map(p => {
                                  if (p.id === pay.id) {
                                    ...
                                    const fRate = Number(p.foreignFeeRate) || val;
                                    const cRate = Number(p.cableFeeRate) || val;
                                    return { 
                                      ...p, 
                                      exchangeRate: val,
                                      amountKrw: Math.round(amt * (val || appliedExRate)),
                                      foreignFeeKrw: p.foreignFee ? Math.round(p.foreignFee * fRate) : p.foreignFeeKrw,
                                      cableFeeKrw: p.cableFee ? Math.round(p.cableFee * cRate) : p.cableFeeKrw
                                    };
                                  }
                                  return p;
                                });
                              ...
                              style={{ width: '85px', height: '28px', padding: '0 6px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '12px', fontWeight: 700, color: '#1e3a8a', outline: 'none' }}
                            />
                            <span style={{ fontSize: '11px', color: '#64748b' }}>₩/$</span>
                          </div>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                            ...
                            <select
                              ...
                              style={{ height: '28px', padding: '0 2px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11px', fontWeight: 700, color: '#334155', background: '#fff' }}
                            >
                              <option value="USD">USD</option>
                              <option value="KRW">KRW</option>
                            </select>
                            <input 
                              type="number"
                              placeholder="외화 (18.00)"
                              value={pay.foreignFee ?? ''}
                              onChange={(e) => {
                                const val = e.target.value === '' ? undefined : Number(e.target.value);
                                const updatedPayments = (request.payments || []).map(p => {
                                  if (p.id === pay.id) {
                                    const r = Number(p.foreignFeeRate) || Number(p.exchangeRate) || appliedExRate;
                              ...
                              style={{ width: '75px', height: '28px', padding: '0 4px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '12px', fontWeight: 600, outline: 'none' }}
                            />
                            <span style={{ fontSize: '11px', color: '#64748b' }}>환율:</span>
                            <input 
                              type="number"
                              placeholder="환율 (1383.00)"
                              value={pay.foreignFeeRate ?? (pay.exchangeRate || '')}
                              onChange={(e) => {
                                const val = e.target.value === '' ? undefined : Number(e.target.value);
                                const updatedPayments = (request.payments || []).map(p => {
                                  if (p.id === pay.id) {
                                    const r = val !== undefined ? val : (Number(p.exchangeRate) || appliedExRate);
                                    const calculatedKrw = p.foreignFee !== undefined ? Math.round(p.foreignFee * r) : p.foreignFeeKrw;
                                    return { ...p, foreignFeeRate: val, foreignFeeKrw: calculatedKrw };
                                  }
                                  return p;
                                });
                                saveToStorage(importRequests.map(r => r.id === id ? { ...r, payments: updatedPayments } : r));
                              }}
                              style={{ width: '80px', height: '28px', padding: '0 4px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '12px', fontWeight: 600, outline: 'none' }}
                            />
                            <span style={{ fontSize: '11px', color: '#64748b' }}>≈ ₩</span>
                            <input 
                              type="number"
                              placeholder="원화 (24894)"
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                            ...
                            <input 
                              ...
                              onChange={(e) => {
                                ...
                                const updatedPayments = (request.payments || []).map(p => {
                                  if (p.id === pay.id) {
                                    const r = Number(p.cableFeeRate) || Number(p.exchangeRate) || appliedExRate;
                                    const calculatedKrw = val !== undefined ? Math.round(val * r) : undefined;
                            ...
                            <input 
                              type="number"
                              placeholder="환율 (1383.60)"
                              value={pay.cableFeeRate ?? (pay.exchangeRate || '')}
                              onChange={(e) => {
                                const val = e.target.value === '' ? undefined : Number(e.target.value);
                                const updatedPayments = (request.payments || []).map(p => {
                                  if (p.id === pay.id) {
                                    const r = val !== undefined ? val : (Number(p.exchangeRate) || appliedExRate);
                                    const calculatedKrw = p.cableFee !== undefined ? Math.round(p.cableFee * r) : p.cableFeeKrw;
                                    return { ...p, cableFeeRate: val, cableFeeKrw: calculatedKrw };
                                  }
                                  return p;
                                });
                                saveToStorage(importRequests.map(r => r.id === id ? { ...r, payments: updatedPayments } : r));
        ...
        {activeTab === '정산' && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              {/* 2-Column Grid: Left (Deal Statement) & Right (Tax Invoice & Collections) */}
              <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '12px', alignItems: 'start' }}>
                
                {/* Left Side: 4-1. 거래명세표 발행 및 관리 */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div style={{ background: '#f0fdf4', padding: '12px', borderRadius: '8px', border: '1px solid #bbf7d0' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #cbd5e1', paddingBottom: '4px', marginBottom: '8px' }}>
                      ...
                      <button
                        ...
                        style={{ padding: '4px 8px', background: '#166534', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}
                      >
                        🖨️ 거래명세표 인쇄/발행
                      </button>
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px', background: '#fff', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        <label style={{ fontSize: '10px', fontWeight: 750, color: '#475569' }}>상호</label>
                        <input
                          type="text"
                          ...
                          style={{ height: '30px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '12px', padding: '0 6px', fontWeight: 600 }}
                          placeholder="회사명"
                        />
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        ...
                        <input
                          ...
                          style={{ height: '30px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '12px', padding: '0 6px' }}
                          placeholder="사업자번호"
                        />
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        <label style={{ fontSize: '10px', fontWeight: 750, color: '#475569' }}>발행일자</label>
                        <input
                          type="date"
                          ...
                          style={{ height: '30px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '12px', padding: '0 4px' }}
                        />
                      </div>
                    ...
                    <div style={{ background: '#fff', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '11px' }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid #cbd5e1', height: '24px', background: '#f8fafc' }}>
                            <th style={{ padding: '2px', width: '25px' }}>월</th>
                            <th style={{ padding: '2px', width: '25px' }}>일</th>
                                ...
                                <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', height: '32px' }}>
                                  <td style={{ padding: '1px' }}>
                                    <input
                                      ...
                                      style={{ width: '100%', height: '24px', border: '1px solid #cbd5e1', borderRadius: '4px', textAlign: 'center', fontSize: '11px' }}
                                    />
                                  </td>
                                  <td style={{ padding: '1px' }}>
                                    <input
                                  ...
                                  <td style={{ padding: '1px', textAlign: 'center' }}>
                                    <button
                                      type="button"
                                      onClick={deleteItem}
                                      style={{ background: 'none', border: 'none', color: '#ef4444', fontWeight: 'bold', cursor: 'pointer', fontSize: '11px' }}
                                    >
                                      ✕
                                    </button>
                                  </td>
                ...
                {/* Right Side: 4-2. 세금계산서 정산 & 4-3. 수금 내역 */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  
                  {/* 4-2. 고객사 세금계산서 정산 */}
                  <div style={{ background: '#eff6ff', padding: '12px', borderRadius: '8px', border: '1px solid #bfdbfe' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-default)', paddingBottom: '4px', marginBottom: '8px' }}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const qty = Number(request.costBreakdown?.buyingQty) || request.piItems?.reduce((sum, it) => sum + (Number(it.qty) || 0), 0) || 1;
                        ...
                        style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', color: '#475569', borderRadius: '4px', fontSize: '10.5px', fontWeight: 700, cursor: 'pointer', padding: '2px 6px' }}
                      >
                        ⚡ 자동계산 입력
                      </button>
                    </div>
                    ...
                    {/* 세금계산서 테이블 */}
                    <div style={{ overflowX: 'auto', background: '#fff', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '550px', fontSize: '11px' }}>
                        <thead>
                          <tr style={{ borderBottom: '1.5px solid #cbd5e1', height: '28px' }}>
                            <th style={{ padding: '2px 4px', width: '80px' }}>구분</th>
                            <th style={{ padding: '2px 4px', width: '100px' }}>발행일자</th>
                            <th style={{ padding: '2px 4px', width: '110px' }}>승인번호</th>
                            <th style={{ padding: '2px 4px', width: '100px', textAlign: 'right' }}>공급가액</th>
                            <th style={{ padding: '2px 4px', width: '80px', textAlign: 'right' }}>부가세</th>
                            <th style={{ padding: '2px 4px', width: '90px', textAlign: 'right' }}>합계</th>
                            <th style={{ padding: '2px 4px', width: '30px', textAlign: 'center' }}></th>
                          </tr>
                        </thead>
                        <tbody>
                          {(() => {
                            ...
                            const updateRow = (rowId: string, fields: Partial<TaxDocumentRow>) => {
                              ...
                              const updatedRequests = importRequests.map(r => r.id === id ? { ...r, ...compatibilityFields } as any : r);
                              saveToStorage(updatedRequests);
                            };

                            const deleteRow = (rowId: string) => {
                              const updatedRows = rows.filter(r => r.id !== rowId);
                              const updatedRequests = importRequests.map(r => r.id === id ? { ...r, taxDocumentRows: updatedRows } as any : r);
                              saveToStorage(updatedRequests);
                            };
                            ...
                            return (
                              <>
                                {rows.map((row) => (
                                  <tr key={row.id} style={{ borderBottom: '1px solid #e2e8f0', height: '36px' }}>
                                    <td style={{ padding: '1px' }}>
                                      <select
                                        value={row.type}
                                        onChange={(e) => updateRow(row.id, { type: e.target.value as any })}
                                        style={{ width: '100%', height: '26px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11px', fontWeight: 600 }}
                                      >
                                        <option value="세금계산서">세금계산서</option>
                                        <option value="영수증">영수증</option>
                                        <option value="기타">기타</option>
                                    ...
                                    <td style={{ padding: '1px' }}>
                                      <input
                                        ...
                                        style={{ width: '100%', height: '26px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11px', padding: '0 2px' }}
                                      />
                                    </td>
                                    <td style={{ padding: '1px' }}>
                                      <input
                                        ...
                                        placeholder="승인번호"
                                        style={{ width: '100%', height: '26px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11px', padding: '0 4px' }}
                                      />
                                    </td>
                                    <td style={{ padding: '1px' }}>
                                      <input
                                        ...
                                        style={{ width: '100%', height: '26px', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '11px', textAlign: 'right', padding: '0 4px' }}
                                      />
                                    </td>
                                    <td style={{ padding: '1px' }}>
                                      <input
                                    ...
                                    <td style={{ padding: '2px', textAlign: 'right', fontWeight: 800 }}>
                                ...
                                <tr>
                                  <td colSpan={7} style={{ padding: '4px 0' }}>
                                    <button
                                      ...
                                      style={{ width: '100%', height: '28px', background: '#fff', border: '1px dashed #cbd5e1', borderRadius: '4px', fontSize: '11px', fontWeight: 700, color: '#64748b', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}
                                    >
                                      ➕ 증빙 추가 (행 추가)
                                    </button>
                                  </td>
                  ...
                  <div style={{ background: '#fff', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px', borderBottom: '1px solid #cbd5e1', paddingBottom: '4px' }}>
                      ...
                      <button
                        ...
                        style={{ padding: '2px 6px', background: '#3b82f6', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '10.5px', fontWeight: 'bold', cursor: 'pointer' }}
                      >
                        ＋ 수금 등록
                      </button>
                    </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '11px' }}>
                      <thead>
                        <tr style={{ borderBottom: '1.5px solid #cbd5e1', height: '24px', background: '#f8fafc' }}>
                          <th style={{ padding: '4px 6px', width: '70px' }}>차수</th>
                          <th style={{ padding: '4px 6px', width: '110px' }}>수금 일자</th>
                          <th style={{ padding: '4px 6px', width: '110px', textAlign: 'right' }}>수금액</th>
                          <th style={{ padding: '4px 6px', width: '110px', textAlign: 'right' }}>잔액</th>
                          <th style={{ padding: '4px 6px' }}>메모</th>
                      ...
                      <tbody>
                        {(() => {
                          ...
                          if (collectionsList.length === 0) {
                            return (
                              <tr>
                                <td colSpan={6} style={{ textAlign: 'center', padding: '8px', color: '#64748b', fontSize: '11px' }}>
                                  등록된 수금 내역이 없습니다.
                                </td>
                              </tr>
                            );
                          ...
                          return collectionsList.map((col) => {
                            ...
                            const updateCol = (fields: Partial<any>) => {
                            ...
                            return (
                              <tr key={col.id} style={{ borderBottom: '1px solid #e2e8f0', height: '32px' }}>
                                <td style={{ padding: '4px 6px', fontWeight: 'bold', color: '#1e3a8a' }}>{col.round}차 수금</td>
                                <td style={{ padding: '1px' }}>
                                  <input
                                    type="date"
                                    value={col.date}
                                    onChange={(e) => updateCol({ date: e.target.value })}
                                    style={{ width: '100%', height: '24px', border: '1px solid #cbd5e1', borderRadius: '4px', padding: '0 4px', fontSize: '11px' }}
                                  />
                                </td>
                                <td style={{ padding: '1px' }}>
                                  <input
                                    ...
                                    style={{ width: '100%', height: '24px', border: '1px solid #cbd5e1', borderRadius: '4px', textAlign: 'right', padding: '0 4px', fontWeight: 'bold', fontSize: '11px' }}
                                    placeholder="수금액"
                                  />
                                </td>
      ...
      {showForwarderModal && (
        <ForwarderSearchModal 
          ...
          onSelect={(supplier) => {
            const updated = importRequests.map(r => r.id === id ? { ...r, forwarderName: supplier.name } : r);
            saveToStorage(updated);
            ...
            {(() => {
              ...
              const printItemsData = (request.piItems && request.piItems.length > 0)
                ? request.piItems.map((item) => {
                    const itemCurrency = item.quoteCurrency || item.currency || 'USD';
